...
import { useMyCommunities, useJoinedCommunities } from "../hooks/useCommunity";
...
export default function HomePage() {
  ...
  // Combine owned and joined communities to get total count
  const totalCommunities = React.useMemo(() => {
    ...
    return owned.length + uniqueJoined.length;
  }, [myCommunities, joinedCommunities]);

  // Check if there are more than 3 communities
  const hasMoreCommunities = totalCommunities > 3;
            ...
            <div className="flex justify-between items-center">
              ...
              {hasMoreCommunities && (
                <button
                  ...
                  className="bg-blue-600 hover:bg-blue-700 text-white py-2 px-4 rounded-md text-sm font-medium transition-colors duration-200"
                >
                  See All ({totalCommunities})
                </button>
              )}
            </div>

            <MyCommunitiesSection limit={3} />
          </div>
        </div>
      </div>
