        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 mb-4 md:mb-0 md:mr-6">
          {userProfile?.avatarUrl ? (
            <img
              ...
              alt="Profile"
              className="w-25 h-25 md:w-30 md:h-30 lg:w-35 lg:h-35 rounded-full object-cover border-2 border-[var(--border)]"
            />
          ) : (
            <div className="w-16 h-16 md:w-20 md:h-20 lg:w-24 lg:h-24 rounded-full bg-gradient-to-br from-[var(--primary)] to-purple-600 flex items-center justify-center text-white text-xl md:text-2xl font-bold">
              {(userProfile?.name || "U")
                .split(" ")
                .map((n: string) => n[0])
                .join("")
